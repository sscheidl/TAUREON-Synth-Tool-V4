...
| R-002 | WinMM long-message teardown/lifetime defect | Medium | High | Production callback→bounded queue→worker requeue and deterministic `MIDIHDR` owners; error-path unit tests; 100 production open/close cycles with zero drops/late callbacks; Stage 1 short/SysEx byte-integrity regression retained; vendor-driver validation remains | Codex + Claude Code review | Open / mitigated architecture; hardware follow-up |
